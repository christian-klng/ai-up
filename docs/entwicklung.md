...
src/app/(app)       eingeloggter Bereich (Start, Wissen, Mitglieder, Meetings*, Nachrichten, Notifications, Workflows, Profil)
src/app/admin       Verwaltung (Allgemein/Branding, Zweck, Mitglieder, Wissensbereiche, Fragen, LLM, Workflows, API-Schlüssel)
src/app/api         auth (Better Auth), files (Medien), upload, events (SSE), mcp, health
src/server/auth     Better-Auth-Konfiguration, Session-Guards
src/server/db       Drizzle-Schema, Client, Migrate/Seed
src/server/domain   Fachlogik (users, settings, knowledge, messenger, notifications, questions, bot, api-keys)
