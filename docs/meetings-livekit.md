...
3. **Konfigurationsdateien**: `egress.yaml` wird unverändert per Bind-Mount eingebunden. `livekit.yaml` im Repo ist eine **Vorlage mit Platzhaltern** (`LIVEKIT_DOMAIN`, `LIVEKIT_API_KEY`, `APP_WEBHOOK_URL`) – diese Datei **nicht im Repo ausfüllen und committen**: Sie enthielte dann den API-Key und die Domain einer konkreten Installation. Stattdessen in Coolify unter *Storages* eine Datei mit Ziel `/etc/livekit.yaml` anlegen und den ausgefüllten Inhalt dort hineinkopieren; das überschreibt den Bind-Mount. Falls die automatische IP-Erkennung fehlschlägt, dabei auch `node_ip` einkommentieren und setzen.
