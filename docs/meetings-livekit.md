...
3. **Konfigurationsdateien**: `egress.yaml` wird unverändert per Bind-Mount eingebunden. `livekit.yaml` im Repo ist eine **Vorlage mit Platzhaltern** (`LIVEKIT_DOMAIN`, `LIVEKIT_API_KEY`, `APP_WEBHOOK_URL`) – diese Datei **nicht im Repo ausfüllen und committen**: Sie enthielte dann den API-Key und die Domain einer konkreten Installation. Stattdessen den ausgefüllten Inhalt in Coolify unter *Persistent Storage* (ältere Versionen: *Storages*) hinterlegen: Der Bind-Mount aus der Compose-Datei erscheint dort als Datei-Mount mit Ziel `/etc/livekit.yaml`, dessen Inhalt sich im Formular bearbeiten lässt. Fehlt der Eintrag, einen neuen Datei-Mount mit diesem Ziel anlegen. Falls die automatische IP-Erkennung fehlschlägt, dabei auch `node_ip` einkommentieren und setzen.
