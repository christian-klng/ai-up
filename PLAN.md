...
> Stand: 24.08.2026 · Status: **Phasen 1–6 + System-Bot umgesetzt und produktiv im Einsatz** – App und Media-Server laufen auf zwei getrennten VPS (Coolify), erster Call mit Audio-Aufzeichnung über S3 am 24.08.2026 verifiziert. Von Phase 4 fehlt nur der Lasttest (4f); **Phase 7 offen** · Arbeitsname „AI-Up“ (Produktname wird vom Admin in der App gesetzt)
>
...
- **Vorlagen** (seit 27.08.2026, vorher „Strukturen“ pro Sammlung): Eigenständige, vom Admin visuell definierte Formulare (`content_templates` + versionierte Snapshots in `content_template_versions`; Definition als JSON, Typen in `src/lib/structures/`). Admins weisen Vorlagen pro Sammlung zu (`knowledge_area_templates`); ohne Zuweisung gelten die geseedeten, nicht löschbaren System-Vorlagen (einfacher Text, Bild, Link, Video). Elemente: Infotext, Textfeld, Langtext, Fließtext (Markdown, optional als Akkordeon-Liste mit mehreren betitelten, umsortierbaren Abschnitten), Dropdown, Chips, Checkbox, Frage-Antwort-Paare, Prozessdiagramm (@xyflow/react, verzweigte Graphen), Bild, Link, Video (Upload oder URL); Bedingungen über `showIf`. Mitglieder wählen beim Anlegen aus den verfügbaren Vorlagen; neue Einträge sind immer Typ `structured`, die Antworten werden deterministisch zu Markdown gerendert (`body_markdown`, Prozesse als Mermaid + Text-Outline) und samt Definitions-Snapshot in `content_versions.meta.structure` gespeichert – Vorlagen-Änderungen brechen alte Einträge nie. Wird eine Vorlage aktualisiert, bietet die Bearbeiten-Seite ein opt-in Upgrade an (Antworten werden per Element-Key übernommen, Unpassendes entfällt sichtbar). Jeder Eintrag ist als Markdown kopier-/downloadbar (Ausgangsmaterial für System Prompts / Skill-Dateien).
...
| 4d Calls *(fertig 22.08.2026, lokal mit zwei Nutzern verifiziert)* | Token-Ausgabe serverseitig, Call-Seite `/meetings/<space>/<id>/call` mit LiveKit-Prefabs (VideoConference / AudioConference, Screen-Share, Chat), Start/Beitreten, Webhook `/api/livekit/webhook` (room_started, participant_joined/left, room_finished) → Status/Teilnehmer + Realtime `meeting.updated` (Sidebar-Punkt, Seiten), „Meeting beenden“ schließt den Raum, „Wieder öffnen“. |
| 4e Aufzeichnung *(fertig 22.08.2026)* | Audio-only-Egress startet automatisch beim ersten Beitritt (wenn Mitschnitt an) oder per Host-Button, stoppt bei Meeting-Ende; `egress_ended` importiert die Datei vom gemeinsamen Volume in `media_files` (Player am Meeting), Status-Chips (läuft/verarbeitet/verfügbar/fehlgeschlagen), Event + Workflow-Trigger `meeting.started/ended/recording.available`. Offen: Aufbewahrungsfrist/Löschung (Phase 7). |
| 4f Abschluss *(teilweise: Produktiv-Deployment auf zwei VPS und erster Call mit Aufzeichnung am 24.08.2026 verifiziert)* | E2E mit zwei Browsern, **Lasttest-Protokoll (offen)**, Doku (Betrieb, Ports, Backup der Aufzeichnungen). |
