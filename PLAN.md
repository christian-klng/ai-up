...
| **7 – Härtung & Betrieb** | Tests (Engine, Auth, Upload), Sicherheits-Review, Backups/Restore-Probe, Performance (Indizes, Caching), Barrierefreiheit-Grundlagen, Doku, Onboarding-Texte. Aufzeichnungen: (a) Quelldatei im Übergabe-Verzeichnis (`RECORDINGS_PATH`) nach erfolgreichem Import löschen (liegt sonst doppelt), (b) optionaler S3-Übergabeweg (Egress → S3-kompatibler Speicher, z. B. Hetzner Object Storage → App-Import), damit App- und Media-Ressource später auf getrennten Servern laufen können; dazu Aufbewahrungsfrist/Löschung für Aufzeichnungen. | Produktivfreigabe. |
