...
import { registerViaJoinLink, type AuthFormState } from "@/server/actions/auth";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

/** Registration on the join page; after sending it turns into the "check your inbox" hint. */
export function JoinForm({ token }: { token: string }) {
  const t = useTranslations("auth.join");
  const ta = useTranslations("auth");
  const [state, action, pending] = useActionState<AuthFormState, FormData>(registerViaJoinLink, { status: "idle" });
  ...
  return (
    <div>
      ...
      <p className="mt-4 text-sm text-muted-foreground">
        {ta("hasAccount")}{" "}
        <Link href={`/login?next=${encodeURIComponent(`/join/${token}`)}`} className="font-medium underline-offset-4 hover:underline">
          {ta("signIn")}
        </Link>
      </p>
    </div>
