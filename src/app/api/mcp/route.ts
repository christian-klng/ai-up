 ...
 * (Authorization: Bearer aiup_…) and is served by a fresh McpServer instance.
 */
async function handle(req: Request): Promise<Response> {
  ...
  if (!auth) {
    return new Response(JSON.stringify({ error: "unauthorized", message: "Provide a valid API key: Authorization: Bearer aiup_…" }), {
      status: 401,
