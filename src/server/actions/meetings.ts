...
export type InviteActionResult = { ok: true; url: string; enabled: boolean } | { ok: false; code: "forbidden" | "notFound" | "unexpected" };

export async function setInviteEnabledAction(input: { meetingId: string; enabled: boolean }): Promise<InviteActionResult> {
  const user = await assertUser();
  ...
  try {
    ...
    return { ok: true, url: invite.url, enabled: invite.enabled };
  } catch {
